...
run_test_file() {
  ...
  for _func in $_functions; do
    ...
    if sh -c "
      ...
      printf "\r\033[K %s %s (%sms)\n" "$(set_text green "✓")" "$(set_text green "$_func")" "$_duration"
      rm "$_output_file"
    else
...
main() {
  ...
  if [ "$G_TOTAL_FAILURES" -eq 0 ]; then
    printf "%s\n" "$(set_text green "PASSED: $G_TOTAL_TESTS tests in ${_duration}s")"
    return 0
  else
    printf "%s\n" "$(set_text red "FAILED: $G_TOTAL_FAILURES out of $G_TOTAL_TESTS tests failed in ${_duration}s")"
    return 1
  fi
}
